...
function App() {
    ...
    function removeTask(taskID: string, todolistId: number) {
        setTodo(todo.map((t, index) => index === todolistId ? {
            ...t,
            tasks: t.tasks.filter(f => f.taskID !== taskID)
        } : t))
    }

    function addTask(title: string, todolistId: number) {
        let newTask = {taskID: v1(), title: title, isDone: false};
        setTodo(todo.map((t, index) => index === todolistId ? {...t, tasks: [newTask, ...t.tasks]} : t))
    ...
    function changeStatus(id: string, isDone: boolean, todolistId: number) {
        setTodo(todo.map((t, index) => index === todolistId ? {
            ...t,
            tasks: t.tasks.map(task => task.taskID === id ? {...task, isDone} : task)
        } : t))
    ...
    function removeTodolist(todolistId: number) {
        setTodo(todo.filter((t, index) => index !== todolistId))
    }
